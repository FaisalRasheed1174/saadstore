...
import { addT0Cart,removeFromCart } from './../actions/cartActions';
...
export const CartScreen = ({match,location,history }) => {
   ...
   useEffect(()=>{
       if(productId){
           dispatch(addT0Cart(productId,qty))
        }
...
{cartItems.map(item =>(
    ...
    <ListGroupItem key={item.product}>
        <Row>
            ...
            as='select'
            value={item.qty}
            onChange={(e)=> dispatch(addT0Cart(item.product,Number(e.target.value)))}
            >
